				...
				function SectionFunctions:Dropdown(Settings, Flag)
					...
					local function CalculateDropdownSize()
						...
						for _, v in ipairs(dropdownFrame:GetChildren()) do
							if v:IsA("GuiObject") and v.Visible and not v:IsA("UIComponent") then
								totalHeight += v.Size.Y.Offset
								visibleChildrenCount += 1
							end
						end

						local spacing = 0
						if visibleChildrenCount > 1 then
							spacing = dropdownFrameUIListLayout.Padding.Offset * (visibleChildrenCount - 1)
						end
